(function() {
    ...
    function getCachedRepo(repoFullName) {
        try {
            const cached = localStorage.getItem(`gh-card-${repoFullName}`);
    ...
    async function fetchRepoData(repoFullName) {
        const cached = getCachedRepo(repoFullName);
        if (cached) return cached;

        let url, options = {};

        if (PROXY_URL) {
            let baseUrl = PROXY_URL.replace(/\/+$/, '');
            url = `${baseUrl}?repo=${encodeURIComponent(repoFullName)}`;
        } else {
        ...
        if (!res.ok) {
            if (res.status === 403 || res.status === 401) {
                throw new Error('API limit reached or unauthorized. Try using a proxy or token.');
            }
            throw new Error(`HTTP ${res.status}`);
        }
        const data = await res.json();
        if (data.message && (data.message.includes('Not Found') || data.message.includes('API rate limit'))) {
            throw new Error(data.message);
        }
        setCachedRepo(repoFullName, data);
        return data;
    }
    ...
    async function processCardElement(el) {
        ...
        try {
            ...
            if (msg.includes('403')) msg = 'API limited, consider using a proxy';
